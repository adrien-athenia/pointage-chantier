...
import type { Pointage, PointageType } from '../types/database'
...
const RELATIONS_SELECT =
  'id, employe_id, chantier_id, type, pointe_at, created_at, latitude, longitude, gps_accuracy, distance_chantier_m, profiles(full_name, email), chantiers(nom)'

export async function listPointagesWithRelations(
  filters: PointageFilters = {},
): Promise<ServiceResult<PointageWithRelations[]>> {
  let query = supabase.from('pointages').select(RELATIONS_SELECT).order('pointe_at', { ascending: false })
